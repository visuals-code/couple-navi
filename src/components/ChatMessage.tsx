      ...
      {isUser ? (
        ...
        <div className="flex flex-col items-start gap-2">
          ...
          <div className="max-w-[85%] md:max-w-[75%] rounded-2xl px-3 py-2 md:px-4 md:py-3 shadow-card bg-card text-foreground border border-border">
            {message.format === "md" ? (
              <ReactMarkdown remarkPlugins={[remarkGfm]} className="prose prose-sm md:prose-base max-w-none break-words">
                {message.content}
              </ReactMarkdown>
            ) : message.format === "html" ? (
              <div className="prose prose-sm md:prose-base max-w-none break-words" dangerouslySetInnerHTML={{ __html: message.content }} />
            ) : (
              <p className="whitespace-pre-wrap text-sm md:text-base leading-relaxed break-words">{message.content}</p>
            )}

            {message.format !== "html" && message.sources && message.sources.length > 0 && (
              <div className="mt-2 border-t border-border pt-2">
                <p className="text-xs text-muted-foreground mb-1">출처</p>
                <ul className="space-y-1">
                  {message.sources.map((s, idx) => (
                    <li key={idx} className="text-xs text-muted-foreground break-all">
                      {s.url ? (
                        <a href={s.url} target="_blank" rel="noreferrer" className="underline hover:opacity-80">
                          {s.title || s.url}
                        </a>
                      ) : (
                        s.title || ""
